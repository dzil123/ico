...
onready var _base_radius = (
	_base.rect_size
	* _base.get_global_transform_with_canvas().get_scale()
	/ 2
)
...
func _ready() -> void:
	if (
		not OS.has_touchscreen_ui_hint()
...
func _move_base(new_position: Vector2) -> void:
	_base.rect_global_position = (
		new_position
		- _base.rect_pivot_offset * get_global_transform_with_canvas().get_scale()
	)


func _move_tip(new_position: Vector2) -> void:
	_tip.rect_global_position = (
		new_position
		- _tip.rect_pivot_offset * _base.get_global_transform_with_canvas().get_scale()
	)


func _is_point_inside_joystick_area(point: Vector2) -> bool:
	var x: bool = (
		point.x >= rect_global_position.x
		and (
			point.x
			<= (
				rect_global_position.x
				+ (rect_size.x * get_global_transform_with_canvas().get_scale().x)
			)
		)
	)
	var y: bool = (
		...
		and (
			...
			<= (
				...
				+ (rect_size.y * get_global_transform_with_canvas().get_scale().y)
			)
		)
	)
	return x and y
